...
def _pacer_bulk_stats(root: Path) -> dict[str, Any]:
    bulk = root / "data" / "collected" / "PACER" / "BULK_FOLDER"
            ...
            if "TOTAL" in line.upper():
                ...
                if parts:
                    try:
                        ...
                        pass
    canonical = root / "data" / "collected" / "PACER"
    canon_dirs = [
        p.name
...
def verify_claim(claim: Claim, ctx: VerifyContext) -> VerifyResult:
  ...
  # --- DB corpus stats ---
    ...
    if claim.id == "cover.pacer_records":
        ...
        return VerifyResult(claim.id, status, f"total PACER dirs={obs}", obs, "8", "data/collected/PACER", notes)
