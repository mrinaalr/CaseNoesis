...
Railway may host the CaseNoesis **website**. Collection and MCP stay **on your machine**. Outputs: `data/collected/{press_releases,pacer,manifests}/<domain>/`. Nothing auto-ingests.
...
CourtListener ─► court_records.py ──► free RECAP PDFs only (never PACER)
...
| `run_bulk.py` | Scale: 100 today, 1000/50 when ready |
...
Or MCP `collect_bulk` (same layout). Scale with `--press-count 1000 --court-count 50`.
...
Or local MCP: `collect_bulk(press_count=1000, court_count=50)` after reloading `.cursor/mcp.json` (`CASENOESIS_API_URL=http://localhost:8000`).
